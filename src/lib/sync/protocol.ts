...
export const SYNC_ENTITIES = ['tombstones', 'settings', 'academy', 'progress', 'shelf', 'documents', 'results', 'scores', 'sessions'] as const;
export type SyncEntity = (typeof SYNC_ENTITIES)[number];
export const TOMBSTONE_ENTITIES = ['results', 'scores', 'sessions', 'progress', 'shelf', 'documents'] as const satisfies readonly SyncTombstoneEntity[];
...
export const sessionRow = z.object({
  clientId: z.uuid(), workKey: z.string().max(300), kind: z.enum(['story', 'book', 'import']), title: z.string().max(400), author: z.string().max(300),
...
/** Records merged last-writer-wins by key: reading progress and shelf entries. */
...
export interface PushResult { progress: KeyedStateRow[]; shelf: KeyedStateRow[]; academy: StateRow | null; settings: StateRow | null }
