// The Academy course: eleven units of guided lessons. Each lesson teaches new keys, drills them in steps, and ends
...
const UNIT_SPECS: UnitSpec[] = [
  ...
  },
  {
    ...
    lessons: [
      ...
      { id: 'shift-2', title: 'Acronyms and title case', goal: 'Hold Shift through a run of capitals, then let go cleanly for the rest of the word.', newKeys: [], tips: ['For a run of capitals, keep Shift held rather than pressing it for every letter.', 'Caps Lock is slower to undo than a held Shift. Leave it off.'], steps: textSteps(['NASA, the BBC and UNESCO sent a team to the UK in May.', 'The Old Man and the Sea sat beside War and Peace on the Tuesday shelf.', 'Dr Ada Byron of the USA met Mr Omar Khan at the NHS office in York.'], ['Acronyms', 'Titles and names']), checkpoint: { wpm: 20, accuracy: 92 } }
    ]
  },
  {
  ...
  },
  {
    ...
    lessons: [
      ...
      { id: 'symbols-1', title: 'Brackets and braces', goal: 'Square brackets, curly braces and angle brackets, opened and closed without looking.', newKeys: ['[', ']', '{', '}', '<', '>'], tips: ['[ and ] belong to the right pinky; add Shift for { and }.', '< and > share the comma and full-stop keys, with Shift.'], steps: textSteps(['[one] [two] {three} {four} <five> <six>', 'Keep notes [in brackets] and code {in braces}; compare a < b and b > c.', 'The list [a, b] and the map {key: value} sit inside <section> tags.'], ['Pairs', 'In sentences']), checkpoint: { wpm: 14, accuracy: 90 } },
      { id: 'symbols-2', title: 'At, hash and friends', goal: 'The marks of email, money and maths: @ # $ & * + = _ | ~', newKeys: ['@', '#', '$', '&', '*', '+', '=', '_', '|', '~'], tips: ['Each of these is Shift plus a number or punctuation key; the opposite hand holds Shift.', 'Say the key you are reaching for, then reach. Guessing costs more than slowing down.'], steps: textSteps(['Email ana@mail.org about order #42 for $15 & tax.', 'Five * four + two = 22, and snake_case | kebab-case ~ camelCase.', 'Send #7 to sam_lee@home.net; pay $30 + $12 = $42 & keep ~10% back.'], ['Everyday marks', 'Maths and names']), checkpoint: { wpm: 14, accuracy: 90 } }
    ]
  },
  {
    id: 'code', title: 'Programming', summary: 'Real lines of code, symbols and all.',
    lessons: [
      { id: 'code-1', title: 'Code basics', goal: 'Type variables, calls and conditions with every symbol in its place.', newKeys: [], tips: ['Code punishes a missing bracket. Accuracy first, speed after.', 'Type the opening and closing marks as a pair in your head.'], steps: textSteps(['let count = 0; count += 1;', 'if (count > 3) { print("done"); }', 'const total = items.map((item) => item.price * 2);'], ['Statements', 'Conditions']), checkpoint: { wpm: 18, accuracy: 92 } },
      { id: 'code-2', title: 'Code in context', goal: 'Longer lines mixing strings, paths, arrays and comments.', newKeys: [], tips: ['Read the whole line first; code is easier once you see its shape.', 'Quotes come in pairs. Close the one you opened.'], steps: textSteps(['git commit -m "add login form" && git push', 'for (let i = 0; i < list.length; i++) { sum += list[i]; } // add up', 'user = {"name": "Ada", "tags": ["math", "code"], "admin": false}'], ['Shell', 'Loops and data']), checkpoint: { wpm: 20, accuracy: 93 } }
    ]
  },
  {
    ...
    lessons: [
      ...
      { id: 'rhythm-3', title: 'Awkward pairs', goal: 'Smooth out the same-finger and long-reach pairs that break a typing rhythm.', newKeys: [], tips: ['Pairs like ed, un and my use one finger twice; give it a moment to travel.', 'Long reaches like br and ce are easier with a relaxed wrist.'], steps: textSteps(['ed ed un un my my ce ce br br ol ol ny ny sw sw', 'The bright nymph swung under the old cedar and bravely smiled.', 'Many unbroken cedars grew beside the swollen brook, and my uncle loved them.'], ['Pairs', 'Pairs in words']), checkpoint: { wpm: 28, accuracy: 94 } }
    ]
  },
  {
  ...
  },
  {
    ...
    lessons: [
      ...
      { id: 'speed-2', title: 'Sprint drills', goal: 'Short, fast phrases made of the most common words, repeated until they feel automatic.', newKeys: [], tips: ['Push for speed on these; they are short enough to stay accurate.', 'Keep the beat even through the spaces; the space bar is a key too.'], steps: textSteps(['of the and to in is it that of the and to in is it that', 'it is in the and it is to the of the and that it is in the', 'and that is the way it is in the end of the day, and it is time to go.'], ['Sprint', 'Faster']), checkpoint: { wpm: 42, accuracy: 95 } }
    ]
  },
  {
