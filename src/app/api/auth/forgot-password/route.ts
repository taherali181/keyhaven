...
export async function POST(request: Request) {
  ...
  if (!await allowAuthAttempt(cloudDb, `reset:${requestAddress(request)}`, 5, 60 * 60_000)) return Response.json({ error: 'Too many attempts. Please try again later.' }, { status: 429 });
  const [user] = await cloudDb.select({ id: users.id }).from(users).innerJoin(credentials, eq(users.id, credentials.userId)).where(eq(users.email, parsed.data.email)).limit(1);
  if (user && process.env.AUTH_RESEND_KEY) {
    const token = randomBytes(32).toString('hex');
    await cloudDb.insert(passwordResetTokens).values({ userId: user.id, tokenHash: createHash('sha256').update(token).digest('hex'), expiresAt: new Date(Date.now() + 30 * 60_000) });
