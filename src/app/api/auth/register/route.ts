...
export async function POST(request: Request) {
  ...
  if (!await allowAuthAttempt(syncDb, `register:${requestAddress(request)}`, 5, 60 * 60_000)) return Response.json({ error: 'Too many attempts. Please try again later.' }, { status: 429 });
  ...
  const existing = await syncDb.select({ id: users.id }).from(users).where(eq(users.email, parsed.data.email)).limit(1);
  if (existing.length) return Response.json({ error: 'An account already exists for this email.' }, { status: 409 });
  const id = crypto.randomUUID();
  await syncDb.insert(users).values({ id, name: parsed.data.name, email: parsed.data.email });
  try {
    await syncDb.insert(credentials).values({ userId: id, passwordHash: await hash(parsed.data.password, 12) });
    await createUniqueProfile(syncDb, id, parsed.data.name);
  } catch (error) {
    await syncDb.delete(users).where(eq(users.id, id));
    throw error;
  }
